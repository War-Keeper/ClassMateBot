...
class Helper(commands.Cog):
    ...
    @commands.dm_only()
    @commands.command(name="verify", pass_context=True, help="Request the bot to verify the user to get access to channels")
    async def verify(self, ctx, *, name : str = None):
        guild = None
        for g in self.bot.guilds:
            if g.name == self.GUILD: guild = g
        ...
        if unverified in member.roles: #checks if the user running the command has the unveirifed role
            if name == None :
                await ctx.send("Please enter $verify your_full_name to get access to channels")
            else:
                verified = discord.utils.get(guild.roles, name=self.VERIFIED_MEMBER_ROLE) #finds the verified role in the guild
                with open('data/server_data/name_mapping.csv', mode='a', newline="") as outfile: #storing discord name and actual name in name_mapping.csv
